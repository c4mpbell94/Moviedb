...
public class MovieCollection : IMovieCollection
{
	...
	{
		//To be completed
		while(movie.CompareTo(ptr.Movie) != 0)
        {
	...
	private IMovie Search(string movietitle, BTreeNode r)
    {
		while (r != null)
		{
			if (r.Movie.Title.CompareTo(movietitle) == 0)
			{
				return r.Movie;
			}
			else
	...
	private void ToArray(BTreeNode root)
    {
		if(root != null)
		{
			ToArray(root.LChild);
			Console.Write(root.Movie);
			ToArray(root.RChild);
		}
	}
