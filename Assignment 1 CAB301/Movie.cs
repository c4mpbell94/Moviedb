...
public class Movie : IMovie
{
    ...
    public bool AddBorrower(IMember member)
    {
        ...
        while (AvailableCopies >= 1 && Borrowers.Search(member) != true)
            {
    ...
    public bool RemoveBorrower(IMember member)
    {
        ...
        while (Borrowers.Search(member) != false)
        {
    ...
    {
        //To be completed
        return (title + " " + genre.ToString() + " " + classification.ToString() + " " + duration.ToString() + " " + availablecopies.ToString() + "\n");
    }
}
